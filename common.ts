...
import { readableStreamToString, SumsubError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new SumsubError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new SumsubError(e, e.response?.data)
                }
                if (innerError instanceof SumsubError) {
                    // Got "SumsubError" from the above try block
                    throw innerError;
                }
